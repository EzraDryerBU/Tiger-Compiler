...
struct
  ...
  fun appBinop T.PLUS i1 i2 = i1 + i2
    ...
    | appBinop _ _ _ = (print "1"; raise Unsupported)
  ...
  fun appRelop T.EQ i1 i2 = i1 = i2
    ...
    | appRelop _ _ _ = (print "2"; raise Unsupported)
  
  fun lookupFS _ [] = (print "3"; raise Unsupported)
      ...
      let
        ...
        and unsafeJump l s = case jump l s
                                of RunStm s' => (interp s'; false)
                                 | RunNextSeq => true
                                 | _ => (print ("label " ^ Symbol.name l ^ " not found"); Printtree.printtree(TextIO.stdOut, stm); (print "4"; raise Unsupported))
        ...
        and interp (T.SEQ(s1, s2)) = (if interp s1 then interp s2 else false)
          ...
          | interp (T.JUMP (_, _)) = (print "5"; raise Unsupported)
          ...
          | interp (T.MOVE(_, _)) = (print "6"; raise Unsupported)
