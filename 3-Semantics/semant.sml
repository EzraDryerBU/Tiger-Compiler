...
struct
  ...
  fun transVar (loop : loopstat) (venv : venv) (tenv : tenv) (v : A.var) : T.ty =
      let
        fun trvar (A.SimpleVar (s, pos)) = 
          ...
          | trvar (A.SubscriptVar(v, exp, pos)) =
              (checkInt (transExp loop venv tenv exp) "array subscript" pos;
                case trvar v
  ...
  and transExp (loop : loopstat) (venv : venv) (tenv : tenv) (e : A.exp) : T.ty =
  let
    fun trexp (A.VarExp v) = transVar loop venv tenv v
      ...
      | trexp (A.AssignExp { var, exp, pos }) =
        (compatType(trexp exp, transVar loop venv tenv var, pos, "incorrect types in assignment", "bindee's type", "bound's type");
      ...
      | trexp (A.WhileExp { test, body, pos}) =
        (checkInt (trexp test) "while test" pos;
         checkUnit (transExp InLoop venv tenv body) "while body" pos;
         T.UNIT)
      | trexp (A.ForExp { var, lo, hi, body, pos, ...}) =
        ...
        (checkInt (trexp lo) "for lower bound" pos;
         ...
         checkUnit (transExp InLoop venv' tenv body) "for body" pos;
         T.UNIT)
        end
      ...
      | trexp (A.LetExp {decs, body, pos}) =
          let
              fun walk vt [] = vt
                | walk {venv, tenv} (d :: ds) = walk (transDec loop venv tenv d) ds
              val { venv = venv2, tenv = tenv2 } = walk {venv = venv, tenv = tenv} decs
          in
          transExp loop venv2 tenv2 body
          end
  ...
  and transDec (loop : loopstat) (venv : venv) (tenv : tenv) (d : A.dec) : { venv : venv, tenv : tenv} =
      case d
        of A.FunctionDec fs =>
            let
              ...
              fun add ({ name, params, result, body, pos } : A.fundec, table) =
                  ...
                  in
                    S.enter(table, name, E.FunEntry { formals = formals, result = res })
                  end
                
              val venv_fs = foldr add venv fs
              
              fun checkBody ({ params, result, body, pos, ... } : A.fundec) =
                let
                  fun addParams ({ name, typ, pos, ... } : A.field, table) =
                      S.enter(table, name, E.VarEntry { ty = lookTy tenv typ pos, readonly = false })
                  val venv_frm = foldr addParams venv_fs params

                  val res = getResult result
                  val body_ty = transExp loop venv_frm tenv body
                in
         ...
         | A.VarDec { name, typ, init, pos, ...} =>
            let 
              val init_ty = transExp loop venv tenv init
            in
            case typ
              of SOME (ty_s, ty_s_pos) =>
                ...
                (compatType(annot_ty, init_ty, pos, "annotated type does not match expression's type", "annotation type", "expression type");
                 { venv = Symbol.enter(venv, name, E.VarEntry { ty = sel_ty, readonly = false }), tenv = tenv })
                end
               | NONE => 
                  ((case init_ty
                      of T.NIL => ErrorMsg.error pos "TYPE: type of nil cannot be inferred" 
                      | _ => ());
                  { venv = Symbol.enter(venv, name, E.VarEntry { ty = init_ty, readonly = false }), tenv = tenv})
            end
         | A.TypeDec ts =>
            let
  ...
  fun transProg e = transExp NotLoop E.base_venv E.base_tenv e
end
