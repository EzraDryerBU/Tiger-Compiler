...
STRING | COMMENT | INITIAL | STRINGWHITE
...
fun yyAction42 (strm, lastMatch : yymatch) = (yystrm := strm;
      (YYBEGIN COMMENT; commNest := 1; continue()))
fun yyAction43 (strm, lastMatch : yymatch) = (yystrm := strm;
      (YYBEGIN STRING; instring := true; string := ""; continue()))
...
fun yyAction49 (strm, lastMatch : yymatch) = let
      ...
      in
        yystrm := strm;
        (let val code = case String.explode yytext of _ :: yy => String.implode yy
...
fun yyAction52 (strm, lastMatch : yymatch) = (yystrm := strm;
      (YYBEGIN INITIAL; instring := false; Tokens.STRING(!string, yypos, yypos + 2 + String.size (!string))))
fun yyAction53 (strm, lastMatch : yymatch) = (yystrm := strm;
      (YYBEGIN STRINGWHITE; continue()))
...
fun yyAction58 (strm, lastMatch : yymatch) = (yystrm := strm;
      (commNest := !commNest + 1; continue()))
fun yyAction59 (strm, lastMatch : yymatch) = (yystrm := strm;
      (if !commNest = 1 then (commNest := 0; YYBEGIN INITIAL; continue())
        ...
        | SOME(inp, strm') =>
            if inp = #"["
              ...
              then if inp = #":"
                  ...
                  then if inp <= #"/"
                      then yyAction45(strm, yyNO_MATCH)
                      else yyQ57(strm', yyMATCH(strm, yyAction45, yyNO_MATCH))
                else if inp <= #"@"
                  then yyAction45(strm, yyNO_MATCH)
                  else yyQ57(strm', yyMATCH(strm, yyAction45, yyNO_MATCH))
            else if inp = #"`"
...
in
  (case (!(yyss))
   of STRING => yyQ0(!(yystrm), yyNO_MATCH)
    ...
    | STRINGWHITE => yyQ3(!(yystrm), yyNO_MATCH)
  (* end case *))
end
            end
	  in 
